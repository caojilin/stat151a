...
printcp(rt)
#This gives the best cross-validated error for various values of cp = alpha. The term relative error is simply RSS(T)/TSS.
#This will therefore always decrease as the tree complexity increases. The xerror is an error calculated by 10-fold cross-validation (and divided by TSS). Because the partition of data into 10 parts is random, xerror is random and xstd provides its standard deviation.
